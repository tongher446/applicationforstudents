'use client';

import { signOut } from 'next-auth/react';
import { useRouter } from 'next/navigation';
import { motion, AnimatePresence } from 'framer-motion';
import Navbar from '../components/navbar';
import Footer from '../components/footer';
export default function WelcomeClient({ session }) {
  const router = useRouter();

  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900">
      <Navbar />
      
      <main className="flex-1 flex flex-col justify-center items-center w-full pt-24 pb-12 px-4">
        <motion.div 
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6 }}
          className="container w-full max-w-2xl border border-blue-300/30 bg-gray-800/50 backdrop-blur-md rounded-2xl shadow-2xl h-96 flex flex-col justify-center items-center p-8"
        >       
          <motion.div 
            className="text-center text-white text-xl font-medium"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ delay: 0.3, duration: 0.5 }}
          >
            Welcome, <span className="text-blue-400 font-bold">{session?.user?.name}</span>!
            <br />
            <span className="text-gray-300 text-lg">You are now in our application system.</span> <br /> 
            <span>Email: </span>
            {session?.user?.email} 
          </motion.div>
          
          {/* Animated welcome decoration */}
          <motion.div
            className="absolute -top-4 -right-4 w-16 h-16 bg-blue-500/20 rounded-full blur-xl"
            animate={{
              scale: [1, 1.2, 1],
              opacity: [0.5, 0.8, 0.5]
            }}
            transition={{
              duration: 3,
              repeat: Infinity,
              ease: "easeInOut"
            }}
          />
          
          <motion.div
            className="absolute -bottom-4 -left-4 w-16 h-16 bg-purple-500/20 rounded-full blur-xl"
            animate={{
              scale: [1, 1.2, 1],
              opacity: [0.5, 0.8, 0.5]
            }}
            transition={{
              duration: 3,
              repeat: Infinity,
              ease: "easeInOut",
              delay: 1
            }}
          />
        </motion.div>
        <button onClick={() => signOut()} className="border-2 border-red-500 bg-white-600 hover:bg-red-700 text-white font-semibold px-6 py-2 rounded-lg mt-8 transition duration-300 ease-in-out hover:scale-105">
          sign out 
        </button>
      </main>
      
      <Footer />
    </div>
  );
}